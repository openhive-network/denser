...
OPTIONS:
  ...
  --hive-rocket-chat-url=HIVE_ROCKET_CHAT_URL     Hive Rocket Chat URL to be used by the new instance (default: 'https://openhive.chat')
...
HIVE_ROCKET_CHAT_URL=${HIVE_ROCKET_CHAT_URL:-"https://openhive.chat"}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --explorer-domain=*)
        ...
        EXPLORER_DOMAIN="$arg"
        ;;
    --hive-rocket-chat-url=*)
        arg="${1#*=}"
        HIVE_ROCKET_CHAT_URL="$arg"
        ;;
    --chain-id=*)
        arg="${1#*=}"
        CHAIN_ID="$arg"
...
RUN_OPTIONS=(
    ...
    "--env" "REACT_APP_HIVE_ROCKET_CHAT_URL=$HIVE_ROCKET_CHAT_URL"
