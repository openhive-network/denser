...
export const loggerStyles = {
  slimConstructor: ['color: white;', 'background-color: #276156;', ...commonStyle].join(' '),
  slimDestructor: ['color: black;', 'background-color: rgb(255, 180, 0);', ...commonStyle].join(' '),
  slimFatal: ['color: white;', 'background-color: rgb(0, 0, 0);', ...commonStyle].join(' '),
  slimError: ['color: white;', 'background-color: rgb(255, 80, 80);', ...commonStyle].join(' '),
  slimWarn: ['color: black;', 'background-color: rgb(255, 255, 153);', ...commonStyle].join(' '),
  slimInfo: ['color: white;', 'background-color: rgb(55,105,150);', ...commonStyle].join(' '),
  slimDebug: ['color: black;', 'background-color: rgb(153, 255, 204);', ...commonStyle].join(' ')
};
...
export class Logger {
  /**
   * Information where to output log messages. Can be only `console` or `noop`
   * at this moment.
   *
   * @private
   * @type {LoggerOutput}
   * @memberof Logger
   */
  private _output!: LoggerOutput;

  get output(): string {
    return this._output;
  }

  set output(value: LoggerOutput) {
    this._output = value;
  }
  ...
  set level(logLevel: keyof LoggerLogLevels) {
