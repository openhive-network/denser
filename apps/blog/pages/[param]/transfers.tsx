...
import { getAccount, getAccountHistory, getDynamicGlobalProperties, getFeedHistory } from '@/blog/lib/hive';
import { delegatedHive, getCurrentHpApr, numberWithCommas, vestingHive } from '@/blog/lib/utils';
...
import { AccountHistory } from '@/blog/store/app-types';
import { GetServerSideProps, InferGetServerSidePropsType } from 'next';
import Link from 'next/link';
import Loading from '@/blog/components/loading';
import TransfersHistoryFilter, { TransferFilters } from '@/blog/components/transfers-history-filter';
...
function TransfersPage({ username }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  if (accountLoading || dynamicLoading || historyFeedLoading) {
    return <Loading loading={accountLoading || dynamicLoading || historyFeedLoading} />;
  }
  ...
  const filteredHistoryList = accountHistoryData?.filter(({ operation }) => {
    ...
    if (operation.type === 'transfer') {
      ...
      return operation.to?.includes(filter.search) || operation.from?.includes(filter.search);
    }
    ...
    if (operation.type === 'claim_reward_balance') {
      ...
      return (
        <span>
          {'Claim rewards: '}
          {displayHBD && (
            <span>{operation.reward_hbd.toString() + ' HBD ' + (displayHIVE ? ', ' : 'and ')}</span>
          )}
          {displayHIVE && <span>{operation.reward_hive.toString() + ' HIVE and '}</span>}
          {powerHP.toFixed(3)}
          {' HIVE POWER'}
        </span>
      );
      ...
      if (operation.to === username)
        return (
          <span>
            ...
            <Link
              href={`/@${operation.from}`}
              className="font-semibold text-zinc-900 hover:text-red-600 dark:text-zinc-100 dark:hover:text-blue-400"
            >
              {operation.from}
            </Link>
          </span>
      ...
      <div>
        <div className="flex gap-6 border-b-2 border-zinc-500 px-4 py-2">
          ...
          <Link href={`/@${username}/delegations`}>
            <div className="hover:text-red-600 dark:hover:text-blue-400">Delegations</div>
          </Link>
        </div>
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2 p-2">
            ...
            <p className="text-xs text-zinc-600">
              Tradeable tokens that may be transferred anywhere at anytime. Hive can be converted to HIVE
              POWER in a process called powering up.
            </p>
            <div className="font-bold">{numberWithCommas(balance_hive.toFixed(3)) + ' HIVE'}</div>
          </div>
          <div className="flex flex-col gap-2 p-2">
            <div className="font-bold">HIVE POWER</div>
            <p className="text-xs text-zinc-600 ">
              Influence tokens which give you more control over post payouts and allow you to earn on curation
              rewards. Part of {accountData?.name}&apos;s HIVE POWER is currently delegated. Delegation is
              donated for influence or to help new users perform actions on Hive. Your delegation amount can
          ...
          <div className="flex flex-col gap-2 p-2">
            ...
            <div className="font-bold">{'$' + numberWithCommas(hbd_balance.toFixed(3))}</div>
          </div>
          <div className="flex flex-col gap-2 p-2">
            <div className="font-bold">SAVINGS</div>
            <p className="text-xs text-zinc-600">
              &quot;Balances subject to 3 day withdraw waiting period. HBD interest rate: 20.00% APR (as voted
              by the{' '}
              <span className="font-semibold text-zinc-900 hover:text-red-600 dark:text-zinc-100 dark:hover:text-blue-400">
            ...
            <div className="flex flex-col font-bold">
              ...
              <span>{numberWithCommas('$' + hbd_balance_savings.toFixed(3))}</span>
            </div>
          </div>
          <div className="flex  flex-col gap-2 p-2">
        ...
        <div className="p-2">
          ...
          <p className="text-xs text-zinc-600">
            Beware of spam and phishing links in transfer memos. Do not open links from users you do not
            trust. Do not provide your private keys to any third party websites. Transactions will not show
            until they are confirmed on the blockchain, which may take a few minutes.
          </p>
        </div>
              ...
              <tbody>
                {filteredHistoryList?.reverse().map((element) => {
                  ...
                  return (
                    ...
                    >
                      <td className=" px-4 py-2 ">{dateToFullRelative(element.timestamp)}</td>
                      <td className=" da px-4 py-2 ">{historyItemDescription(element.operation)}</td>
                      <td className="break-all px-4 py-2 ">
                        {element.operation.type === 'transfer' ? element.operation.memo : null}
                      </td>
                    </tr>
                  );
                })}
