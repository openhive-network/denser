...
export class ProfilePage {
  ...
  constructor(page: Page) {
    ...
    this.profileLastTimeActive = page.locator(
      '[data-testid="user-last-time-active"]'
    )
    ...
    this.postsMenuPostsButton = page
      .locator('[data-testid="user-post-menu"]')
      .getByText("Posts")
    this.postsMenuCommentsButton = page
      .locator('[data-testid="user-post-menu"]')
      .getByText("Comments")
    this.postsMenuPayoutsButton = page
      .locator('[data-testid="user-post-menu"]')
      .getByText("Payouts")

    this.repliesCommentListItem = page.locator(
      '[data-testid="comment-list-item"]'
    )

    this.notificationsMenu = page.locator(
    ...
    this.notificationsMenuReblogsButton = page
      .locator('[data-testid="notifications-local-menu"]')
      .getByText("Reblogs")
    this.notificationsMenuAllContent = page.locator(
      '[data-testid="notifications-content-all"]'
    )

    this.publicProfileSettings = page.locator(
      '[data-testid="public-profile-settings"]'
    )

    this.thirdPartyAppPeakdLink = page.locator('a[href="https://peakd.com/"]')
    this.thirdPartyAppHivebuzzLink = page.locator(
      'a[href="https://hivebuzz.me/"]'
    )
  }
  ...
  async profileFeedTabIsSelected() {
    ...
    expect(
      await this.getElementCssPropertyValue(this.profileFeedLink, "color")
    ).toBe("rgb(220, 38, 38)")
    expect(
      await this.getElementCssPropertyValue(
        this.profileFeedLink,
        "background-color"
      )
  ...
  async profilePostsTabIsSelected() {
    await this.page.waitForSelector(this.postFeedItem["_selector"])
    expect(
      await this.getElementCssPropertyValue(this.profilePostsLink, "color")
    ).toBe("rgb(220, 38, 38)")
    expect(
      await this.getElementCssPropertyValue(
        this.profilePostsLink,
        "background-color"
      )
    ).toBe("rgb(254, 226, 226)")
    await expect(this.postFeedItem).toHaveCount(20)
  ...
  async profileRepliesTabIsSelected() {
    ...
    expect(
      await this.getElementCssPropertyValue(this.profileRepliesLink, "color")
    ).toBe("rgb(220, 38, 38)")
    expect(
      await this.getElementCssPropertyValue(
        this.profileRepliesLink,
        "background-color"
      )
    ).toBe("rgb(254, 226, 226)")
  ...
  async profileSocialTabIsSelected() {
    ...
    expect(
      await this.getElementCssPropertyValue(this.profileSocialLink, "color")
    ).toBe("rgb(220, 38, 38)")
    expect(
      await this.getElementCssPropertyValue(
  ...
  async profileSettingsTabIsSelected() {
    ...
    expect(
      await this.getElementCssPropertyValue(this.profileSettingsLink, "color")
    ).toBe("rgb(220, 38, 38)")
    expect(
      await this.getElementCssPropertyValue(
  ...
  async moveToWalletPage() {
    ...
    await expect(this.page).toHaveURL(
      /https:\/\/wallet.hive.blog\/@gtg\/transfers$/
    )
