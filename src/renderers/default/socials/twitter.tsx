...
const regex = {
    ...
    htmlReplacement:
        /<blockquote[^>]*?><p[^>]*?>(.*?)<\/p>.*?mdash; (.*)<a href="(https:\/\/twitter\.com\/.*?(.*?\/status\/(.*?))\?.*?)">(.*?)<\/a><\/blockquote>/i,
};
 ...
 */
export function extractMetadataFromEmbedCode(data: string) {
    if (!data) return null;
 ...
 * @returns {boolean|*}
 */
export function validateIframeUrl(url: string) {
...
function generateTwitterCode(
    metadata: WithImplicitCoercion<string> | { [Symbol.toPrimitive](hint: "string"): string },
) {
    let twitterCode =
 ...
 * @returns {*}
 */
export function genIframeMd(idx: any, twitterId: any, w: any, h: any, metadata: any) {
 ...
 * @returns {*}
 */
export function embedNode(child: { data: any }) {
    try {
        const { data } = child;
 ...
 * @returns {string}
 */
export function preprocessHtml(child: string) {
    try {
