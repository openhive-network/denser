...
const postArticleSchema = Yup.object()
    .shape({
        ...
        content: Yup.string()
            ...
            .max(
            10,
            ({ max }) => `Character limit for content exceeded! Max ${max} characters allowed!`
            ),
...
const createArticle: NextApiHandler<PostResponse> = async (req, res) => {
  ...
  const data = await postArticleSchema.validate(req.body);
