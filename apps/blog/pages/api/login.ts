...
import { sessionOptions } from '@/blog/lib/session';
import { apiHandler } from '@/blog/lib/api';
...
const verifyLoginChallenge = async (
  ...
  const verify = (
    ...
    if (weight !== 1 || weight_threshold !== 1) {
      ...
      if (!verified) {
        logger.error(`verifyLoginChallenge signature verification failed for user ${chainAccount.name}`);
      }
      return verified;
    }
  };
