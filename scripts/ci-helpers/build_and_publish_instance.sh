...
[[ -z "$TURBO_APP_SCOPE" ]] && echo "Option '--app-scope' must be set" && print_help && exit 1
[[ -z "$TURBO_APP_PATH" ]] && echo "Option '--app-path' must be set" && print_help && exit 1
[[ -z "$TURBO_APP_NAME" ]] && echo "Option '--app-name' must be set" && print_help && exit 1
...
"$SRC_DIR/scripts/build_instance.sh" "$SRC_DIR" \
    --tag="$CI_COMMIT_TAG" \
    --registry="$CI_REGISTRY_IMAGE" \
    --progress=plain \
    --app-scope="$TURBO_APP_SCOPE" \
    --app-path="$TURBO_APP_PATH" \
    --app-name="$TURBO_APP_NAME"

# On CI the finished image is directly pushed to the registry, so it needs to be pulled to be tagged
docker pull "$CI_REGISTRY_IMAGE/$TURBO_APP_NAME:$CI_COMMIT_TAG"

echo "Tagging the image built in the previous step as hiveio/$CI_PROJECT_NAME:$CI_COMMIT_TAG"
docker tag "$CI_REGISTRY_IMAGE/$TURBO_APP_NAME:$CI_COMMIT_TAG" "hiveio/$CI_PROJECT_NAME/$TURBO_APP_NAME:$CI_COMMIT_TAG"

echo "Tagging the image built in the previous step as registry.hive.blog/$CI_PROJECT_NAME:$CI_COMMIT_TAG"
docker tag "$CI_REGISTRY_IMAGE/$TURBO_APP_NAME:$CI_COMMIT_TAG" "registry.hive.blog/$CI_PROJECT_NAME/$TURBO_APP_NAME:$CI_COMMIT_TAG"
