...
function parseReport(reportPath: string | undefined, appName: string): ReportData | null {
  ...
  try {
    ...
    console.error(`Failed to parse ${reportPath}:`, message);
    return null;
  }
}
...
function formatScoreCell(score: number, threshold?: number): string {
...
function formatReport(data: ReportData | null): string | null {
  if (!data) return null;
...
function buildComment(blogData: ReportData | null, walletData: ReportData | null): string {
  const parts = [
    COMMENT_MARKER,
    "## 🚦 Lighthouse Results",
    ""
...
function apiRequest(method: string, requestPath: string, body: unknown = null): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const url = new URL(`${CI_API_V4_URL}${requestPath}`);
    ...
    const req = lib.request(options, (res) => {
      ...
      res.on("end", () => {
        if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
          ...
          reject(new Error(`API ${method} ${requestPath} failed: ${res.statusCode} ${data}`));
        }
      });
    });
...
async function findMergeRequestIid(): Promise<string | null> {
  if (mergeRequestIid) {
  ...
  const requestPath = `/projects/${encodeURIComponent(CI_PROJECT_ID!)}/merge_requests?source_branch=${encodeURIComponent(CI_COMMIT_REF_NAME)}&state=opened&per_page=1`;

  try {
    const mrs = await apiRequest("GET", requestPath) as Array<{ iid: number; title: string }>;
    if (mrs && mrs.length > 0) {
      console.log(`Found MR !${mrs[0].iid}: ${mrs[0].title}`);
      return String(mrs[0].iid);
    }
    console.log("No open MR found for this branch");
    return null;
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.warn("Failed to find MR:", message);
    return null;
  }
}

async function deleteOldComments(mrIid: string): Promise<void> {
  const requestPath = `/projects/${encodeURIComponent(CI_PROJECT_ID!)}/merge_requests/${mrIid}/notes?per_page=100`;

  try {
    const notes = await apiRequest("GET", requestPath) as Array<{ id: number; body?: string }>;
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.warn("Failed to delete old comments:", message);
  }
}

async function postComment(mrIid: string, body: string): Promise<void> {
  const requestPath = `/projects/${encodeURIComponent(CI_PROJECT_ID!)}/merge_requests/${mrIid}/notes`;
  await apiRequest("POST", requestPath, { body });
