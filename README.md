...
Block log, alongside with its accompanying alternative chain spec, can be obtained from a special Docker image using the following command:

```bash
./haf/scripts/ci-helpers/export-data-from-docker-image.sh registry.gitlab.syncad.com/hive/hive/extended-block-log:42bf3ac5 /full/path/to/directory/containing/the/block_log/file --image-path=/blockchain/
```
...
./scripts/setup-stack.sh \
  ...
  --haf-arguments="--chain-id=44 --skeleton-key=5JNHfZYKGaomSFvd4NUdQ9qMcEAC43kujbfjueTHpVapX1Kzq2n --replay-blockchain --stop-at-block 5000785" \
  --alternate-chain-spec="/full/path/to/directory/containing/the/block_log/file/alternate-chain-spec.json" \ # If your block log requires an alternative chain spec, ptovide its path here
...
```bash
./scripts/start-stack.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser
```

and wait for HAF replay and Hivemind sync to finish:

```bash
./scripts/wait-for-stack.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser --wait-for-sync=5000785
```
...
./scripts/stop-stack.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser
...
./scripts/start-stack.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser
./scripts/wait-for-stack.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser
```
...
```bash
TZ=UTC faketime -f "@[timestamp]" /opt/google/chrome/chrome --ignore-certificate-errors
```
...
For example if the timestamp of the latest block is 2016-09-16T07:27:09, then the command should be as follows (note that 'T' in the timestamp was replaced by a space):

```bash
TZ=UTC faketime -f "@2016-09-16 07:27:09" /opt/google/chrome/chrome --ignore-certificate-errors
```
...
./scripts/get-stack-logs.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser

# Get the last 10 lines logs per service for all services
./scripts/get-stack-logs.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser --tail=10

# If you want to know the service names, the easiest way is to run
./scripts/get-stack-logs.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser --tail=1
...
./scripts/get-stack-logs.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser --service=haf

# Save the logs to file
./scripts/get-stack-logs.sh --env-files=/full/path/to/mirronet-stack.env --project-name=mirrornet-api-stack --profiles=denser --no-color 2>&1 > mirrornet-api-stack.log
```
