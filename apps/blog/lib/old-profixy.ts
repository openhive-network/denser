...
export const imageProxy = () => `${envData.NEXT_PUBLIC_IMAGES_ENDPOINT}`;
...
export const proxifyImageUrl = (url: string, dimensions: string | boolean) => {
  ...
  if (dimensions && `${envData.NEXT_PUBLIC_IMAGES_ENDPOINT}`) {
    let dims = dimensions + '/';
    ...
    if ((NATURAL_SIZE !== dims && CAPPED_SIZE !== dims) || !rProxyDomain.test(respUrl)) {
      return `${envData.NEXT_PUBLIC_IMAGES_ENDPOINT}` + dims + respUrl;
    }
  }
  return respUrl;
};
