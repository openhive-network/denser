...
export const useTradeHistory = (config?: useTradeHistoryOptions) => {
  return useQuery(["recentTrades"], () => getRecentTrades(), {
    ...
    select: (data) => {
      return data.map((e) => ({
        ...e,
        hbd: convertStringToBig(
          e.current_pays.includes("HBD") ? e.current_pays : e.open_pays
        ),
        hive: convertStringToBig(
          e.current_pays.includes("HIVE") ? e.current_pays : e.open_pays
        ),
      }));
    },
  });
