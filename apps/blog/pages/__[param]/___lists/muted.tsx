...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  return {
    props: {
      metadata: await getAccountMetadata((ctx.params?.param as string) ?? '', 'Muted by'),
      ...(await getTranslations(ctx))
    }
  };
};
export default function Muted({ metadata }: { metadata: MetadataProps }) {
  const { username } = useSiteParams();
  const mutedQuery = useFollowListQuery(username, 'muted');
  if (mutedQuery.isLoading) {
    return <div>Loading</div>;
  }
  return <ProfileLists username={username} variant="muted" data={mutedQuery.data} metadata={metadata} />;
}
