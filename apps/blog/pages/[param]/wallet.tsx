...
import { getAccountHistory, getSavingsWithdrawals } from '@/blog/lib/wallet/hive';
import {
  createListWithSuggestions,
  getAmountFromWithdrawal,
  getCurrentHpApr,
  getFilter
} from '@/blog/lib/wallet/utils';
...
import { TransferDialog } from '@/blog/components/wallet/transfer-dialog';
import useFilters from '@/blog/components/wallet/hooks/use-filters';
import { getAccountMetadata, getTranslations } from '@/blog/lib/wallet/get-translations';
import FinancialReport from '@/blog/components/wallet/financial-report';
import { useClaimRewardsMutation } from '@/blog/components/wallet/hooks/use-claim-rewards-mutation';
import { useMemo, useState } from 'react';
import { useCancelPowerDownMutation } from '@/blog/components/wallet/hooks/use-power-hive-mutation';
import env from '@beam-australia/react-env';
import { useCancelTransferFromSavingsMutation } from '@/blog/components/wallet/hooks/use-cancel-transfer-from-savings-mutation';
      ...
      case 'transfer_from_savings':
        return (
          <span>
            ...
            <Link href={`/@${operation.to}`} className="font-semibold text-secondary hover:text-destructive">
              {`${operation.to} `}
            </Link>
        ...
        if (operation.to === username)
          return (
            <span>
              ...
              <Link
                href={`/@${operation.from}`}
                className="font-semibold text-secondary hover:text-destructive"
              >
                {operation.from}
              </Link>
            </span>
              ...
              <tbody>
                <tr className="flex flex-col py-2 sm:table-row">
                  <td className="px-2 sm:px-4 sm:py-4">
                    ...
                    <p
                      className="text-xs leading-relaxed text-secondary/70"
